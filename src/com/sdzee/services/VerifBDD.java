...
public class VerifBDD extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException{
	 ...
	 public static void verifBDD(HttpServletRequest request, HttpServletResponse response) {
		 try {
				Connection con = (Connection) DatabaseConnection.initializeDatabase();
				
				// Create a SQL query to insert data into demo table 
	            // demo table consists of two columns, so two '?' is used 
				String reqSQL="SELECT u.login,u.mdp FROM utilisateur AS u WHERE u.login="+login+" AND u.mdp="+mdp+" ";
...
//	            st.setString(2, mdp); 
	            ...
	            while (rs.next()) {
					...
					if (loginUtilisateur.equals(login) && mdpUtilisateur.equals(mdp)) {
